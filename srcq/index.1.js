...
var QiyuAdaptor = module.exports = {
    ...
    answer:  function(options) {
        if(adaptor.session) {
            answer(adaptor.session, options);
        }
    },
    bye: function(options) {
        if(adaptor.session) {
            bye(adaptor.session, options);
        }
    },
    sendDigit: function(tone) {
        if(adaptor.session) { 
            sendDigit(adaptor.session, tone);
        }
    },
...
function onRegistrationFailed(data){
    ...
    var isConnected = adaptor.ua.isConnected();
